...
{	
	...
	/**
 	* path completo, o directorio donde esta la imagen
 	* @var		String
 	* @access 	private
 	*/
	private $sourcePath; // romel
	
	/**
 	* path completo, o directorio donde se colocara la imagen
 	* @var		String
 	* @access 	private
 	*/
	private $targetPath; // romel
	...
	{	if(in_array($propiedad,$this->noPermitirSetVar))
		...
		{	
			// romel
			if($propiedad == 'imagenOrigen'){
				$this->$propiedad	= $this->sourcePath . $valor;
				return true;
			}
			elseif($propiedad == 'imagenDestino'){
				$this->$propiedad	= $this->targetPath . $valor;
				return true;
			}else{
	...
	{	if(empty($imgDestino))
		...
		{	
			//$this->resultado	= true;
			$this->resultado  = str_replace($this->targetPath,'',$this->imagenDestino);
			return true;
		}
		return false;
	}
	...
	{
		// start custom  1) debe heredear el path para no escribri sobre el www. 2) creamos un archivo tmp con nombre aleatorio.  <-  by romel ->	
		$ImgTemporal= $this->targetPath."temporal_clase_Imagen_.".strtolower($this->extencion);
		
		$targetPath =	'img/uploads/tmp/';
				
		$pathinfo = pathinfo($ImgTemporal);
		...
		// imagina - cada cliente llama esta funcion 100 veces las probalidades de sobre escribir el archivo tmp aumentan. 
		// no sobre-escribe un archivo | en este caso el archivo tmp -   
        if(!$replaceOldFile){
		...
		$randval = rand();
		$ImgTemporal = $targetPath.$filename.$randval.'.'.$ext;
		//debug($ImgTemporal);
